...
public class AddObjectsToChannelTest extends SeatsioClientTest {
    ...
    public void addObjects() {
        ...
        client.events.channels.add(event.key(), "channelKey2", "channel 2", "#FFFF99", 2, Set.of("A-3", "A-4"), null);

        client.events.channels.addObjects(event.key(), "channelKey1", Set.of("A-3", "A-4"), null);

        Event retrievedEvent = client.events.retrieve(event.key());
        assertThat(retrievedEvent.channels()).containsExactly(
                new Channel("channelKey1", "channel 1", "#FFFF98", 1, Set.of("A-1", "A-2", "A-3", "A-4"), Map.of()),
                new Channel("channelKey2", "channel 2", "#FFFF99", 2, Set.of(), Map.of())
        );
    }
