...
public class SaveSocialDistancingRulesetsTest extends SeatsioClientTest {
    ...
    public void saveFixedRuleset() {
        ...
        Chart retrievedChart = client.charts.retrieve(chart.key);
        assertThat(retrievedChart.socialDistancingRulesets).isEqualTo(ImmutableMap.of("ruleset1", new SocialDistancingRuleset(0, "My first ruleset", 0, false, 0, 0, 0, 0, true, newHashSet("A-1"), newHashSet())));
    }
    ...
    public void saveRuleBasedRuleset() {
        Chart chart = client.charts.create();
        Map<String, SocialDistancingRuleset> rulesets = ImmutableMap.of(
                "ruleset1", SocialDistancingRuleset.ruleBased(0, "My first ruleset", 1, true, 2, 1, 10, 0, newHashSet("A-1"), newHashSet("A-2"))
        );

        client.charts.saveSocialDistancingRulesets(chart.key, rulesets);

        Chart retrievedChart = client.charts.retrieve(chart.key);
        assertThat(retrievedChart.socialDistancingRulesets).isEqualTo(ImmutableMap.of("ruleset1", new SocialDistancingRuleset(0, "My first ruleset", 1, true, 2, 1, 10, 0, false, newHashSet("A-1"), newHashSet("A-2"))));
    }
}
