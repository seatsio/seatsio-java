...
public class HoldTokens {
    ...
    public HoldToken create() {
        HttpResponse<String> response = stringResponse(post(baseUrl + "/hold-tokens", secretKey, workspaceKey));
    ...
    public HoldToken retrieve(String holdToken) {
        HttpResponse<String> response = stringResponse(get(baseUrl + "/hold-tokens/{holdToken}", secretKey, workspaceKey)
    ...
    public HoldToken expireInMinutes(String holdToken, int minutes) {
        ...
        HttpResponse<String> response = stringResponse(post(baseUrl + "/hold-tokens/{holdToken}", secretKey, workspaceKey)
                .routeParam("holdToken", holdToken)
                .body(request.toString()));
        return gson().fromJson(response.getBody(), HoldToken.class);
    }
